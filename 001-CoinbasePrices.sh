#!/bin/bash

# <xbar.title>Coinbase Prices</xbar.title>
# <xbar.version>v1</xbar.version>
# <xbar.author>Fahad Usman</xbar.author>
# <xbar.author.github>fahadshery</xbar.author.github>
# <xbar.desc>Displays Coinbase buy prices of Bitcoin, Ethereum, Litecoin.</xbar.desc>
# <xbar.dependencies>bash, python</xbar.dependencies>
#
# Based on Cryptocurrency Prices plugin by viiraj (github.com/viiraj).
#
# images as base64 strings (look at Parameters in the README file of the xbar github page)

bitcoin_icon=''
eth_icon=''
ltc_icon=''

bit_price_gbp=$(curl -s -H "CB-Version: 2015-04-08" "https://api.coinbase.com/v2/prices/BTC-GBP/buy" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['amount'])" | awk '{print int($1)}' )
bit_price=$(curl -s -H "CB-Version: 2015-04-08" "https://api.coinbase.com/v2/prices/BTC-USD/buy" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['amount'])" | awk '{print int($1)}')
eth_price=$(curl -s -H "CB-Version: 2015-04-08" "https://api.coinbase.com/v2/prices/ETH-GBP/buy" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['amount'])" | awk '{print int($1)}')
ltc_price=$(curl -s -H "CB-Version: 2015-04-08" "https://api.coinbase.com/v2/prices/LTC-GBP/buy" | python3 -c "import sys, json; print(json.load(sys.stdin)['data']['amount'])" | awk '{print int($1)}')


echo "£$bit_price_gbp | templateImage=$bitcoin_icon"
echo \$"$bit_price | templateImage=$bitcoin_icon"
echo "---"
echo "£$eth_price | templateImage=$eth_icon"
echo "---"
echo "£$ltc_price | templateImage=$ltc_icon"
